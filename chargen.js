...
function chooseGender() {
    ...
    console.log(" ________   __     __     ______     ______     _______   ________   ___    __     _    _       __");
    console.log("/ _______| |  |   |  |   |  __  |   |   _  \\   |  _____| |  ______| |   \\  |  |   | \\__/ |     / _\\");
    console.log("| |        |  |___|  |  |  |__|  |  |  |_|  |  | |  ___  | |_____   | |\\ \\ |  |   |  __  |    /  \\");  
    console.log("| |        |   ___   |  |  ____  |  |      /   | | |__ | |  _____|  | | \\ \\|  |   |_/||\\_|   /    \\"); 
    console.log("| |______  |  |   |  | |  |    |  | |  |\\  \\   | |___| | | |______  | |  \\ \\  |      ||     /      \\");
    ...
    let gender = "";
    rl.question('First of all should your character be male or female or diverse? ', (answer) => {
        if(answer === ""){
            gender = "male";
        } else {
            gender = answer;
        ...
        rl.question('So your character will be ' + gender + ". Is that correct? If not please correct your Answer here, else press just enter. ", (answer) => {
            if (answer !== "") {
                gender = answer;
            } 
...
function chooseName(rl) {
    let name = "";
    rl.question('In the next step, you have to choose a name for your character. What name should it be? ', (answer) => {
        if(answer === ""){
            name = "John McCormack";
        } else {
            name = answer;
        ...
        rl.question(`So the name is ${name}? `, (answer) => {
            if (answer === "no") {
                rl.question('Please choose a name for your character. What name should it be? ', (answer) => {
                    name = answer;
                });
            }
            character.name = name;
...
function setCharacteristics(rl) {
    ...
    let age = 18;
    let whand = "right";
    console.log("Now the height, stature, age and the weaponhand of the character are choosen. If you want a random value to be choosen, just press enter.")
    rl.question('Please choose the height of the character. Type the value in cm. ', (answer) => {
        ...
        rl.question('Now choose the stature of your character. You can choose between slim, normal and if not an elf - wide. ', (answer) => {
            if(character.race === "elf"){
                if (answer === "") {
                    ...
                    switch (rdm) {
                        case 1: stature = "slim";
                            break;
                        case 2: stature = "normal";
                            break;
                        default: stature = "slim";
                    }
                } else {
                    stature = answer;
                }
            ...
            rl.question('Now choose the age of your character. It should be at least 16 years. ', (answer) => {
                if (answer === "") {
                ...
                if(character.race === "elf"){age = age*5};
                if(character.race === "dwarf"){age = age*2};
                if(character.race === "gnome"){age = age*4};
                character.age = age;
...
function chooseRace(rl) {
    let race = "";
    rl.question('You can decide which Race your character should have. Your character could be a human, an elf, a dwarf, a halfling or a gnome. ', (answer) => {
        if (answer === "") {
...
function setMoreSkills(rl) {
    ...
    wk = Math.round((Math.random() * 100 + 1) + 2 * (character.ko / 10 + character.wi / 10) - 20);
    switch (character.race) {
        case "elf":
            if (au < 81) { au = 81; }
            break;
...
function chooseClass(rl) {
    let adClass = "";
    let clType = "";
    console.log("Now you have to choose your adventure-class. There are three types of classes. Warriors, Magicians and magical Warriors.\nA list of the different classes is below:")
    console.log("--------------------------------------------------------")
    console.log("Warriors               mag. Warriors       Wizards      ")
    console.log("--------------------------------------------------------")
    console.log("Assasin(As)            Bard(Ba)            Druid(Dr)")
    console.log("Barbarian(Ba)          Warrior(Order)(Or)  Warlock(Hx)")
    ...
    console.log("--------------------------------------------------------")
    console.log(`Your character is a ${character.race}. Depending on your race the choice of your class is limited. \nOnly as a human you are allowed to choose every class.`);
    switch (character.race) {
        case "elf":
    ...
    rl.question("Which class do you choose? Type in the Shortcut. ", (answer) => {
        ...
        if (answer !== "") {
            ...
            adClass = answer;
            if(adClass === "As" || adClass === "Ba" || adClass === "Gl" || adClass === "Hä" || adClass === "Kr" || adClass === "Sp" || adClass === "Wa"){
                clType = "Warrior";
...
function setState(rl){
    ...
    if(character.class === "Dr" || character.class === "Ma"){ rdm = rdm + 10};
    if(character.class === "As" || character.class === "Hä" || character.class === "Wa"){ rdm = rdm - 10};
    if(character.class === "Sp"){ rdm = rdm - 20};
...
function chooseRank(rl) {
    let rank = 0
    console.log("Great! Most of the main information of your character have been added. \nNow your Rank is choosen. Futhermore Lifepoints and Endurancepoints are calculated.");
    rl.question("If your character should have Rank 1 just type Enter. ", (answer) => {
        console.log("-------------------------")
        if (answer === "") {
            rank = 1;
            character.rank = rank;
            console.log(`Your (${character.class}) ${character.name} has rank ${character.rank}.`)
            console.log("**********************************************************************************************************************");
            calculateLPAP(rl);
        } else {
            if (isNaN(answer)) {
                console.log("Sorry, but your input seems to be not a number. Please check your input again.")
                chooseRank(rl);
            } else {
                rank = answer;
                character.rank = rank
...
function calculateLPAP(rl) {
    ...
    if (character.class === "Ba" || character.class === "Kr" || character.class === "Wa") {
        ap = ap + 2;
    }
    if (character.class === "As" || character.class === "Gl" || character.class === "Hä" || character.class === "Sp" || character.class === "Ba" || character.class === "Or" || character.class === "Sc") {
        ap = ap + 1;
    }
    for (let i = 0; i < character.rank; i++) {
        if (character.rank < 6) {
            ...
            } else if (character.class === "As" || character.class === "Gl" || character.class === "Hä" || character.class === "Sp" || character.class === "Ba" || character.class === "Or" || character.class === "Sc") {
...
function calculateBoni(rl) {
    ...
    switch (character.race) {
        case "Dwarf":
            b = Math.floor((Math.random() * 3 + 1) + (Math.random() * 3 + 1) + (Math.random() * 3 + 1)) + 12;
            break;
        case "Halfling":
        ...
        case "Gnome":
            ...
            break;
        default:
            b = Math.floor((Math.random() * 3 + 1) + (Math.random() * 3 + 1) + (Math.random() * 3 + 1) + (Math.random() * 3 + 1)) + 16;
    }
...
function sumUpInformations(rl) {
    console.log("**********************************************************************************************************************");
    rl.question("Do you want to save your character sheet in a file? ", (answer) => {
        if(answer === "yes" || answer === "y"){
            saveSheetInFile();
            rl.close();
        } else {
            rl.question("Are you sure? All your settings will be deleted. Continue? ", (answer) => {
                if(answer === "yes" || answer === "y"){
                    rl.close();
                    character = {};
                    console.log("**********************************************************************************************************************");
...
function setContentToFile(){
    ...
    "***************************************************************************************************************************\n"+
    "***************************************************************************************************************************\n"+
    "NAME: " + `${character.name}`.padStart(22) + "\tGENDER: " + `${character.gender}`.padStart(12) + "\tAGE: " + `${character.age}`.padStart(13) + "\n" +
    "RACE: " + `${character.race}`.padStart(22) + "\tCLASS: " + `${character.class}`.padStart(13) + "\tRANK: " + `${character.rank}`.padStart(12) + "\n" +
    "MILIEU: " + `${character.status}`.padStart(20) + "\tRELIGION: \t\t\t" + "\tHOMETOWN: \t\t\t\n" + 
    "---------------------------------------------------------------------------------------------------------------------------\n" +
    "STATURE: " + `${character.stature}`.padStart(19) + "\tHEIGHT: " + `${character.height}`.padStart(12) + "\tWEAPONHAND: " + `${character.whand}`.padStart(6) + "\n" +
    "***************************************************************************************************************************\n" +
    "STRENGTH: " + `${character.st}`.padStart(18) + "\tDEXTERITY: " + `${character.gs}`.padStart(9) + "\tAGILITY: " + `${character.gw}`.padStart(9) + "\n" +
    "ENDURANCE: " + `${character.ko}`.padStart(17) + "\tINTELLIGENCE: " + `${character.wi}`.padStart(6) + "\tMAGIC TALENT: " + `${character.zt}`.padStart(4) + "\n" +
    "---------------------------------------------------------------------------------------------------------------------------\n" +
    "APPEARENCE: " + `${character.au}`.padStart(16) + "\tCHARISMA: " + `${character.pa}`.padStart(10) + "\tWILL POWER: " + `${character.wk}`.padStart(6) + "\n" +
    "***************************************************************************************************************************\n" +
    "LP: " + `${character.lp}`.padStart(3) + "\n\n" +

    "AP: " + `${character.ap}`.padStart(3) + "\n\n" +

    "***************************************************************************************************************************\n" +
    "MOVEWIDTH: " + `${character.b}`.padStart(17) + "\tENDURANCEBONUS: " + `${character.ab}`.padStart(4) + "\n" +
    "DAMAGEBONUS: " + `${character.schb}`.padStart(15) + "\tATTACKBONUS: " + `${character.anb}`.padStart(7) + "\n" +
    "DEFENSEBONUS: " + `${character.abb}`.padStart(14) + "\tMAGICBONUS: " + `${character.zaub}`.padStart(8) + "\n" + 
    "MIND RESISTENCE: " + `${character.resbg}`.padStart(11) + "\tBODY RESISTENCE: " + `${character.resbk}`.padStart(3) + "\n" +
    "***************************************************************************************************************************\n" +
    "ABILITIES:\n" +
    "---------------------------------------------------------------------------------------------------------------------------\n" +
    "---------------------------------------------------------------------------------------------------------------------------\n" +     
